<template>
  <div class="tickets-filter sidebar__filter-block">
    <div class="tickets-filter__title-block">
      ...
      <button class="tickets-filter__clear-button" @mouseover="togglePopover" @mouseout="togglePopover" @click="clearFilter">
    ...
    <div class="tickets-filter__options options-list">
      <div v-if="isAllCheck" class="options-list__item">
        <input id="opt-airlines-all" class="input__check options-list__checkbox" checked type="checkbox" v-model="isSelectedAll" @change="selectAll">
        <label for="opt-airlines-all" class="options-list__label">Все</label>
      </div>
      <div v-for="{value, label} in options" :key="value" class="options-list__item">
        <input :id="'opt-airlines-' + value" class="input__check options-list__checkbox" type="checkbox" checked :value="value" v-model="selectedOptions">
        <label :for="'opt-airlines-' + value" class="options-list__label">{{ label }}</label>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted: function () {
    if (this.isAllCheck) this.selectedOptions = this.getValues()
  },
  methods: {
    getValues () {
      const values = []
    ...
    selectAll () {
      ...
      if (selectedOptionsCount === optionsCount) {
        this.selectedOptions = []
      } else {
        this.selectedOptions = this.getValues()
      }
    },
    clearFilter () {
      this.selectedOptions = []
